...
class commentVC: UIViewController, UITextViewDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    // values for reseting UI to defaul
    ...
    func textViewDidChange(textView: UITextView) {
        ...
        // disable button if no text is entered
    ...
    func loadComments() {
        ...
        countQuery.countObjectsInBackgroundWithBlock ({ (count:Int32, error:NSError?) -> Void in
            ...
            // if comments on the server for current post are more than (page size 15), implement pull to reshresh func
            if self.page < count {
    ...
    @IBAction func sendBtnClick(sender: AnyObject) {
        ...
        profileArray.append(PFUser.currentUser()?.objectForKey("profile") as! PFFile)
        ...
        commentObj["profile"] = PFUser.currentUser()?.valueForKey("profile")
    ...
    func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [UITableViewRowAction]? {
        ...
        let delete = UITableViewRowAction(style: .Normal, title: "    ") { (action:UITableViewRowAction, indexPath:NSIndexPath) -> Void in
            ...
            self.profileArray.removeAtIndex(indexPath.row)
